      ...
      for (int hint = 0; hint < NHINTS; hint++)
      {
         ...
         if (game.hintlc[hint] >= hints[hint].turns)
         {
            ...
            case 2:            /* snake */
               ...
               return;
            case 3:            /* maze */
               if (game.atloc[game.loc] == NO_OBJECT && game.atloc[game.oldloc] == NO_OBJECT && game.atloc[game.oldlc2] == NO_OBJECT && game.holdng > 1)
                  break;
               game.hintlc[hint] = 0;
               return;
            case 4:            /* dark */
            ...
            case 6:            /* urn */
               ...
               return;
            case 7:            /* woods */
               if (game.atloc[game.loc] == NO_OBJECT && game.atloc[game.oldloc] == NO_OBJECT && game.atloc[game.oldlc2] == NO_OBJECT)
                  break;
               return;
            case 8:            /* ogre */
               i = atdwrf(game.loc);
...
/*  Okay, he's dead.  Let's get on with it. */
{
   ...
   if (game.closng)
   {
      ...
      terminate(endgame);
   } else if (!yes(query, yes_response, arbitrary_messages[OK_MAN]) || game.numdie == NDEATHS)
   {
...
{
   return (travel[a].condtype == travel[b].condtype) && (travel[a].condarg1 == travel[b].condarg1) && (travel[a].condarg2 == travel[b].condarg2) && (travel[a].desttype == travel[b].desttype) && (travel[a].destval == travel[b].destval);
}
      ...
      {                         /* L12 loop */
         ...
         } else
         {
            ...
            case 1:
               ...
               game.newloc = (game.loc == LOC_PLOVER) ? LOC_ALCOVE : LOC_PLOVER;
 ...
 *  get full score. */
{
   if (!DARK(game.loc))
   {
      ...
      for (int i = game.atloc[game.loc]; i != 0; i = game.link[i])
      {
         ...
         if (obj == STEPS)
            kk = (game.loc == game.fixed[STEPS]) ? STEPS_UP : STEPS_DOWN;
...
{
   if (command->word[0].type == MOTION && command->word[0].id == ENTER && (command->word[1].id == STREAM || command->word[1].id == WATER))
   {
   ...
   } else
   {
      if (command->word[0].type == OBJECT)
      {
         ...
         if (command->word[0].id == GRATE)
         {
            ...
            if (game.loc == LOC_COBBLE || game.loc == LOC_DEBRIS || game.loc == LOC_AWKWARD || game.loc == LOC_BIRD || game.loc == LOC_PITTOP)
            {
...
/* Actually execute the move to the new location and dwarf movement */
{
    ...
    * pitch darkness. */
   if (!FORCED(game.loc) && DARK(game.loc) && game.wzdark && PCT(35))
   {                            // FIXME: magic number
      rspeak(PIT_FALL);
      game.oldlc2 = game.loc;
      croak();
...
/* Get and execute a command */
{
   ...
   while (command.state != EXECUTED && !gameover)
   {
      describe_location();
      ...
      clear_command(&command);
      while (command.state <= GIVEN && !gameover)
      {
