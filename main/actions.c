...
static phase_codes_t
attack (command_t command)
 ...
 *  enemies, or no enemies but 2 others. */
{
   ...
   if (obj == VEND)
   {
      state_change (VEND, game.prop[VEND] == VEND_BLOCKS ? VEND_UNBLOCKS : VEND_BLOCKS);
...
{
   if ((game.foobar == WORD_EMPTY && id == FEE) || (game.foobar == FEE && id == FIE) || (game.foobar == FIE && id == FOE)
       || (game.foobar == FOE && id == FOO) || (game.foobar == FOE && id == FUM))
   {
      game.foobar = id;
      if ((id != FOO) && (id != FUM))
      {
 ...
 *  status of bottle.  Also various side effects, etc. */
{
   if (obj == INTRANSITIVE)
   {
      ...
      if (game.atloc[game.loc] == NO_OBJECT || game.link[game.atloc[game.loc]] != 0 || atdwrf (game.loc) > 0)
         return GO_UNKNOWN;
   ...
   if ((obj == BIRD || obj == CAGE) && (game.prop[BIRD] == BIRD_CAGED || STASHED (BIRD) == BIRD_CAGED))
   {
 ...
 *  Drop coins at vending machine for extra batteries. */
{
   ...
   if (GSTONE (obj) && AT (CAVITY) && game.prop[CAVITY] != CAVITY_FULL)
   {
      ...
      if (HERE (RUG) && ((obj == EMERALD && game.prop[RUG] != RUG_HOVER) || (obj == RUBY && game.prop[RUG] == RUG_HOVER)))
      {
         if (obj == RUBY)
            rspeak (RUG_SETTLES);
         else if (TOTING (RUG))
   ...
   if (obj == VASE)
   {
      ...
      {
         state_change (VASE, AT (PILLOW) ? VASE_WHOLE : VASE_DROPPED);
         if (game.prop[VASE] != VASE_WHOLE)
            game.fixed[VASE] = IS_FIXED;
         drop (obj, game.loc);
         return GO_CLEAROBJ;
...
static phase_codes_t
drink (verb_t verb, obj_t obj)
/*  Drink.  If no object, assume water and look for it here.  If water is in
 *  the bottle, drink that, else must be at a water loc, so drink stream. */
{
   if (obj == INTRANSITIVE && LIQLOC (game.loc) != WATER && (LIQUID () != WATER || !HERE (BOTTLE)))
   {
      return GO_UNKNOWN;
   }
...
static phase_codes_t
extinguish (verb_t verb, obj_t obj)
 ...
 *  is nasty.) */
{
   ...
   state_change (BOTTLE, (LIQLOC (game.loc) == OIL) ? OIL_BOTTLE : WATER_BOTTLE);
   if (TOTING (BOTTLE))
      game.place[LIQUID ()] = CARRIED;
   return GO_CLEAROBJ;
}
...
/* Find.  Might be carrying it, or it might be here.  Else give caveat. */
{
   ...
   if (AT (obj) || (LIQUID () == obj && AT (BOTTLE)) || obj == LIQLOC (game.loc) || (obj == DWARF && atdwrf (game.loc) > 0))
   {
      rspeak (YOU_HAVEIT);
      return GO_CLEAROBJ;
   }
...
/*  Read.  Print stuff based on objtxt.  Oyster (?) is special case. */
{
   ...
   } else if (objects[command.obj].texts[0] == NULL || game.prop[command.obj] == STATE_NOTFOUND)
   {
...
/*  Z'ZZZ (word gets recomputed at startup; different each game). */
{
   ...
   {
      state_change (RESER, game.prop[RESER] == WATERS_PARTED ? WATERS_UNPARTED : WATERS_PARTED);
      if (AT (RESER))
         return GO_CLEAROBJ;
      else
      {
...
/* Say.  Echo WD2. Magic words override. */
{
   ...
   if (command.word[1].type == ACTION
       && (command.word[1].id == FEE || command.word[1].id == FIE || command.word[1].id == FOE || command.word[1].id == FOO
...
static phase_codes_t
seed (verb_t verb, const char *arg)
/* Set seed */
{
...
static phase_codes_t
waste (verb_t verb, turn_t turns)
/* Burn turns */
{
   game.limit -= turns;
...
static phase_codes_t
wave (verb_t verb, obj_t obj)
/* Wave.  No effect unless waving rod at fissure or at bird. */
{
   if (obj != ROD || !TOTING (obj) || (!HERE (BIRD) && (game.closng || !AT (FISSURE))))
   {
      speak (((!TOTING (obj)) && (obj != ROD || !TOTING (ROD2))) ? arbitrary_messages[ARENT_CARRYING] : actions[verb].message);
      return GO_CLEAROBJ;
   }
   ...
   } else
   {
      ...
      state_change (FISSURE, game.prop[FISSURE] == BRIDGED ? UNBRIDGED : BRIDGED);
      return GO_CLEAROBJ;
   }
}

phase_codes_t
action (command_t command)
 ...
 */
{
   ...
   if (command.part == unknown)
   {
      ...
      } else if (command.obj == ROD && HERE (ROD2))
      {
         command.obj = ROD2;
         /* FALL THROUGH */ ;
      } else if ((command.verb == FIND || command.verb == INVENTORY)
