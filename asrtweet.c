...
{                               // Expand a tweet (main tweet, quoted tweet, retweeted tweet... (does not line wrap at this point)
   ...
   while (*i)
   {
      ...
      if (check ("&amp;", "&") || check ("&lt;", "<") || check ("&gt;", ">") || check ("£", "GBP") || check ("“", "\"")
   ...
   {                            // POPT
      ...
      while (1)
      {
         ...
         if (id)
         {
            ...
            if (debug)
               warnx ("Following %s: @%s %s (%s) with %s followers.", id, j_get (j, "screen_name"), j_get (j, "name"),
                      j_get (j, "description"), j_get (j, "followers_count"));
         }
      }
      j_delete (&j);
      fclose (i);
